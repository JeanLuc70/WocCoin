...
const char * const PIVX_CONF_FILENAME = "woc.conf";
const char * const PIVX_PID_FILENAME = "woc.pid";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\woc
// Windows >= Vista: C:\Users\Username\AppData\Roaming\woc
// Mac: ~/Library/Application Support/woc
// Unix: ~/.woc
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WoC";
#else
    fs::path pathRet;
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty woc.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override woc.conf
